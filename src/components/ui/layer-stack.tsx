  ...
  React.useEffect(() => {
    const frame = requestAnimationFrame(() => {
      if (!isMobile || containerWidth === 0) {
        setAutoHeight((prev) => (prev !== stageHeight ? stageHeight : prev));
        return;
      }
      const heights = cardRefs.current.map((el) => el?.scrollHeight ?? 0);
      const max = Math.max(...heights);
      if (max > 0) {
        setAutoHeight((prev) => (prev !== max + 32 ? max + 32 : prev));
      }
    });
    ...
    <div ref={ref} className={cn('w-full select-none', className)}>
      ...
      <div className='mt-8 flex flex-col items-center gap-2'>
        <Progress
          ...
          className='w-60 cursor-pointer border border-border-soft-val bg-white/[0.02]'
        />
        <p className='text-[10px] font-bold tracking-[0.18em] uppercase text-text-muted-gray'>
