...
// Inline types replacing the unpublished topojson-specification package
  ...
  useEffect(() => {
    ...
    if (renderGraticule && finalGraticuleOpacity > 0) {
      try {
        ...
        const graticulePathStr = path(graticuleObj());
        if (graticulePathStr) {
          svg
            .append('path')
            .datum(graticuleObj())
            .attr('d', graticulePathStr)
            .attr('fill', 'none')
        ...
        console.error('Error creating graticule');
      }
    }
    ...
    try {
      ...
      console.error('Error creating sphere outline');
    }
  }, [
    worldData,
    progress,
