...
import type { HomeService } from '@/content/home/services';
...
export function ServiceCard({ service }: ServiceCardProps) {
  ...
  return (
    <Link
      ...
      data-service-card
      className="group service-card relative isolate flex min-h-[175px] sm:min-h-[190px] lg:min-h-[205px] xl:min-h-[215px] overflow-hidden rounded-radius-xl border border-border-soft-val bg-white/[0.035] p-4 shadow-shadow-card backdrop-blur-2xl transition-[border-color,background-color,color,opacity,box-shadow] duration-300 hover:border-brand-primary/45 hover:shadow-[0_0_30px_rgba(249,126,26,0.12)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-primary/60"
    >
      ...
      <div className="relative z-10 flex h-full w-full flex-col text-left">
        ...
        >
          <div ref={iconRef} className="flex items-center justify-center">
            <ServiceIcon name={service.icon} className="h-5 w-5 sm:h-6 sm:w-6" aria-hidden="true" />
          </div>
        </div>
        ...
        <div className="mt-3 flex items-center gap-2 font-sans text-xs sm:text-sm font-bold text-brand-primary transition-colors group-hover:text-brand-secondary">
